-- Script corregido para cambiar CodUserBot de INT IDENTITY a NVARCHAR editable
-- Primero verificamos el nombre real de la restricción

-- Paso 1: Obtener el nombre real de la restricción PRIMARY KEY
...
PRINT 'Nombre de la restricción PRIMARY KEY: ' + @PKConstraintName;

-- Paso 2: Eliminar la restricción PRIMARY KEY usando el nombre real
DECLARE @DropConstraintSQL NVARCHAR(MAX) = 'ALTER TABLE UsersBot DROP CONSTRAINT [' + @PKConstraintName + ']';
EXEC sp_executesql @DropConstraintSQL;

-- Paso 3: Eliminar la columna CodUserBot
ALTER TABLE UsersBot DROP COLUMN CodUserBot;

-- Paso 4: Agregar la columna con el nuevo tipo
ALTER TABLE UsersBot ADD CodUserBot NVARCHAR(20) NOT NULL;

-- Paso 5: Asignar códigos a los usuarios existentes
UPDATE UsersBot 
SET CodUserBot = CASE 
...
-- Paso 6: Agregar la restricción PRIMARY KEY
ALTER TABLE UsersBot ADD CONSTRAINT PK_UsersBot PRIMARY KEY (CodUserBot);

-- Paso 7: Actualizar la tabla de auditoría
